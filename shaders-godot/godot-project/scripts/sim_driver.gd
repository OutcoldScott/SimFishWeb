...
func _play_ambient_event(event_name: String, intensity: float = -1.0, species: String = "") -> void:
	...
	# far too often to be useful, so it's intentionally excluded. (Deaths are
	# emitted from _play_ambient_death, which carries richer context.)
	if event_name == "birth" or event_name == "spawn":
		sim_event.emit(event_name, {"species": species})
	var audio := _ambient_audio()
...
func _play_ambient_death(node: Node, cause: String = "") -> void:
	...
	# Telemetry: log every death regardless of whether audio is available.
	sim_event.emit("death", {"species": species, "cause": cause})
	var audio := _ambient_audio()
	if audio == null:
		return
	var age01: float = -1.0
