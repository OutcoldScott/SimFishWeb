...
static func try_load(host: Node, sim: Node, world: Node, aquascape: AquascapeController,
	...
	var seed_info: Dictionary = {}
	if sim != null and sim.get("tank_seed") != null:
		seed_info = {"seed": "%08x" % int(sim.tank_seed)}
	if not FileAccess.file_exists(path):
		# No save for this slot: the world spawned a fresh population.
		if host.has_method("_push_event_to_js"):
			host.call("_push_event_to_js", "tank_new", seed_info)
		return
	...
	if host.has_method("_push_event_to_js"):
		host.call("_push_event_to_js", "tank_load", seed_info)
